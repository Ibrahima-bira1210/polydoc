...
<{% if tag %}{{ tag }}{% else %}div{% endif %} id="div_{{ field.auto_id }}"
    ...
    {% if not field|is_checkboxselectmultiple and not field|is_radioselect %}
        {% if field|is_checkbox and form_show_labels %}
            ...
            {% endif %}
            <label for="{{ field.id_for_label }}"
                   class="{% if use_custom_control %}custom-control-label{% else %}form-check-label{% endif %}{% if field.field.required %} requiredField{% endif %}">
                {{ field.label|safe }}{% if field.field.required %}<span class="asteriskField">*</span>{% endif %}
            </label>
            {% include 'bootstrap4/layout/help_text_and_errors.html' %}
        {% elif field|is_file and not field|is_clearable_file and use_custom_control %}
